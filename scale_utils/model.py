...
class RNNVAE(nn.Module):
  def __init__(self, vocab_size=10000,
    ...
    self.enc_rnn = nn.LSTM(enc_word_dim, enc_h_dim, num_layers = enc_num_layers,
                           batch_first = True)
    nn.init.uniform_(self.enc_word_vecs.weight, -0.01, 0.01)
    # Encoder
    ...
    self.dec_rnn = nn.LSTM(dec_input_size, dec_h_dim, num_layers = dec_num_layers,
                           batch_first = True)    
    nn.init.uniform_(self.dec_word_vecs.weight, -0.01, 0.01)
    # Linear
  ...
  def decoder_forward(self, sent, q_z, init_h = True):
